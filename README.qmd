...
```{r}
# exemple for hist
# to do similarly for metrics if evaluation could be done with pandas and then table collected in R for ease of computation
vroom::vroom("results/evaluation/histograms.tsv") %>% 
  filter(variable == "pr", area == "Vanuatu") %>% 
  ggplot(aes(bin, count, fill = as.factor((month)))) +
  geom_col(col = NA) +
  facet_wrap(origin ~ type, scales = "free_y") +
  theme_bw() +
