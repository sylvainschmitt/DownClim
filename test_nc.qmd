...
```{r}
rcp85 <- rast(list.files(file.path(path, "cmip5_esgf", "proj_ds"),
                pattern = "rcp85", full.names = TRUE)) %>% 
  mean() %>% 
  mask(vect(nc)) %>% 
  crop(rast(xmin=163.5, xmax=168.5, ymin=-22.5, ymax=-19.5))
temp_pal <- c("#053061", "#f7f7f7", "#67001f")
ggplot() +
  tidyterra::geom_spatraster(data = (rcp85/10 - 273.15)) +
    scale_fill_gradient2(low = temp_pal[1], mid = temp_pal[2], 
                       high = temp_pal[3], midpoint = 25,
                       "January\nTemperature\nat surface (°C)", na.value = NA) +
  theme_bw() 
```
