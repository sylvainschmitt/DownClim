...
lapply(files, function(file)
       rast(file.path(path, "cordex_esgf", "merged", file)) %>% 
         crop(nc_bb) %>% 
         subset(grepl("pr", names(.))) %>% 
         subset(year(time(.)) %in% historical) %>% 
         tapp(months, mean) %>% 
         writeRaster(file.path(path, "cordex_esgf", "historical",  
                        paste0("pr_", gsub(".nc", ".tif", file))))
       )
```
...
chelsa <- terra::extract(eval, points) %>% 
  ...
  mutate(variable = "tas", type = "CHELSA",
                     gcm = "", rcm = "", rcp = "")

raw_files <- list.files(file.path(path, "cordex_esgf", "evaluation"), 
           pattern = "tas", full.names = TRUE)
names(raw_files) <- list.files(file.path(path, "cordex_esgf", "evaluation"), 
...
ds_files <- list.files(file.path(path, "cordex_esgf", "eval_ds"), 
           pattern = "tas", full.names = TRUE)
names(ds_files) <- list.files(file.path(path, "cordex_esgf", "eval_ds"), 
           pattern = "tas", full.names = FALSE)
ds <- ds_files %>% 
  lapply(rast) %>% 
  lapply(terra::extract, points) %>% 
  ...
  separate(file, c("variable", "gcm", "rcm", "rcp"), "_") %>% 
  mutate(rcp = gsub(".tif", "", rcp)) %>% 
  mutate(type = "downscaled")

bind_rows(chelsa, raw, ds) %>% 
  write_tsv(file.path(path, "cordex_esgf", "eval_pdf.tsv"))

chelsa %>% 
  select(-gcm, -rcm, -rcp, -type) %>% 
  rename(observed = value) %>% 
  left_join(ds %>% 
              select(-type) %>% 
              rename(downscaled = value)) %>% 
  left_join(raw %>% 
              select(-type) %>% 
              rename(raw = value)) %>% 
  gather(type, predicted, -ID, -month, -observed, -variable, -gcm, -rcm, -rcp) %>% 
  write_tsv(file.path(path, "cordex_esgf", "eval_op.tsv"))
```

```{r pdf}
#| warning: false
vroom::vroom(file.path(path, "cordex_esgf", "eval.tsv")) %>% 
  ggplot(aes(value/10-273.15, group = paste(type, gcm, rcm, rcp),
             col = type)) +
  geom_density(alpha = 0.1) +
  geom_density(data = vroom::vroom(file.path(path, "cordex_esgf", "eval.tsv")) %>% 
                 filter(type == "CHELSA"), 
               aes(col = "CHELSA"), linewidth = 2) +
  facet_wrap(~ month) + 
  theme_bw() + xlab("Temperature (°C)") + ylab("") + xlim(0, 30)
```
...
vroom::vroom(file.path(path, "cordex_esgf", "eval_op.tsv")) %>% 
  na.omit() %>% 
  group_by(variable, gcm, rcm, rcp, type) %>% 
...
ggplot() +
  ...
  theme_bw() +
  ggtitle("Mean evaluation downscaled")
```
